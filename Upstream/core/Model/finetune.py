...
def train_one_epoch(model: torch.nn.Module, optimizer: torch.optim.Optimizer,
    ...
    for data_iter_step, (samples, targets) in enumerate(
            metric_logger.log_every(data_dict["data_loader_train"], print_freq, header)):
        # we use a per iteration (instead of per epoch) lr scheduler
        if data_iter_step % accum_iter == 0:
            lr_sched.adjust_learning_rate(optimizer,
                                          data_iter_step / len(data_dict["data_loader_train"]) + current_epoch, args)
...
def evaluate(model, data_dict, device, task, mode, num_class, threshold=0.5, multi_label=False):
    ...
    print(
        'Sklearn Metrics - Acc: {:.4f} AUC-roc: {:.4f} AUC-pr: {:.4f} F1-score: {:.4f} MCC: {:.4f}, sensitivity: {:.4f} specificity: {:.4f}'.format(
            acc, auc_roc, auc_pr, F1, mcc, sensitivity, specificity))
