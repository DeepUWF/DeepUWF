...
def get_args_parser():
    parser = argparse.ArgumentParser('DeepUWF', add_help=False)
    parser.add_argument('--seed', default=0, type=int)

    # Decentralized federated-learning parameters.  Each site runs one process
    # and exchanges only a random subset of floating-point model tensors.
    parser.add_argument('--federated', action='store_true', help='run one MQTT federated client')
    parser.add_argument('--client-id', type=str, default=None, help='unique client/site identifier')
    parser.add_argument('--run-id', type=str, default='deepuwf', help='shared experiment/run identifier')
    parser.add_argument('--share-fraction', type=float, default=0.2,
                        help='fraction of floating-point state tensors transmitted each round')
    parser.add_argument('--mqtt-host', type=str, default='localhost')
    parser.add_argument('--mqtt-port', type=int, default=1883)
    parser.add_argument('--mqtt-topic', type=str, default='deepuwf')
    parser.add_argument('--mqtt-qos', type=int, choices=(0, 1, 2), default=1)
    parser.add_argument('--mqtt-wait-seconds', type=int, default=60)
    parser.add_argument('--min-peer-updates', type=int, default=1)
    parser.add_argument('--mqtt-tls', action='store_true', help='use mutual TLS for MQTT')
    parser.add_argument('--mqtt-ca-cert', type=str, default=None)
    parser.add_argument('--mqtt-cert', type=str, default=None)
    parser.add_argument('--mqtt-key', type=str, default=None)
    parser.add_argument('--mqtt-tls-insecure', action='store_true',
                        help='disable broker hostname verification; for testing only')
    ...
    parser.add_argument('--pretrain', action='store_true', default=False,
                        help='Run masked image-modeling pretraining')
    parser.add_argument('--finetune', action='store_true', default=False,
                        help='Run supervised downstream fine-tuning')
