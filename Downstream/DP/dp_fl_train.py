"""
Main DP Federated Learning Training Script for DeepUWF
集成 DP-SGD 和 Federated Learning，使用 SH_DR/shrdr 数据集
"""
import os
import sys
...
def create_data_loaders(
    ...
    if datatype==0:
        ...
        from eyedataset import EyeSubjectDataset
        dataset = EyeSubjectDataset(
...
def export_softmax_results(
    ...
    """
    Export softmax probabilities for every sample in ``data_loader`` to a
    JSON file compatible with ``MIA_attack/case.py::load_softmax_results``.

    The JSON structure is:
        {
            "class_0/example_a.png": [p0, p1],
            "class_1/example_b.png": [p0, p1]
        }

    For datatype=0, ``data_loader`` is the existing train_loader built from
    the ``train_dataset`` returned by ``random_split``. Therefore only the
    samples actually used by the training split are exported.
    """
    if data_loader is None:
        raise ValueError("data_loader is None; cannot export softmax results.")
        ...
        with torch.no_grad():
            for batch_index, batch in enumerate(data_loader):
                if len(batch) == 3:
                    ...
                    raise ValueError(
                        "Expected batch format (images, labels, paths) or "
                        "(images, labels, subject_ids, paths), but received "
                ...
                if logits.ndim != 2:
                    raise RuntimeError(
                        f"Expected model logits with shape [B, C], got {tuple(logits.shape)}."
                    )
                ...
                if probabilities.shape[0] != len(paths):
                    raise RuntimeError(
                        "Batch size mismatch between model outputs and paths: "
                        f"{probabilities.shape[0]} vs {len(paths)}."
                    )

                labels_cpu = labels.detach().cpu().tolist()

                for sample_index, raw_path in enumerate(paths):
                    source_path = Path(str(raw_path)).expanduser()

                    # Prefer a stable path relative to dataset_root, e.g.
                    # class_0/xxx.png. This prevents filename collisions
                    # between different class directories.
                    key = None
                    if dataset_root_path is not None:
                        try:
                            key = str(
                                source_path.resolve().relative_to(dataset_root_path)
                            )
                    ...
                    if key is None:
                        ...
                        if parent_name:
                            key = str(Path(parent_name) / source_path.name)
                        else:
                            key = source_path.name

                    if key in softmax_results:
                        raise RuntimeError(
                            "Duplicate softmax JSON key detected: "
                            f"{key}. Source path: {source_path}"
                        )

                    softmax_results[key] = [
                        float(value)
                        for value in probabilities[sample_index].tolist()
                    ]
                    num_samples += 1

                if batch_index % 50 == 0:
                    print(
                        f"[Softmax export] Processed {num_samples} samples "
                        f"through batch {batch_index}."
                    )
    finally:
        model.train(old_training_mode)

    # Sort keys to make the file deterministic. case.py also sorts keys when
    # loading, so the resulting format is directly compatible.
    softmax_results = dict(sorted(softmax_results.items()))
    ...
    print("=" * 80)
    print("Softmax export finished")
    print("=" * 80)
    print(f"Exported samples: {num_samples}")
    print(f"Output JSON: {output_path.resolve()}")

    if softmax_results:
        first_key = next(iter(softmax_results))
        print(
            f"Example: {first_key} -> "
            f"{softmax_results[first_key]}"
        )

    return num_samples, str(output_path)
...
def main(args):
    ...
    if args.role == "server":
        ...
        if args.modedp == "softmax":
            if datatype != 0:
                raise ValueError(
                    "Softmax export mode currently expects datatype=0."
                )
            ...
            # =========================================================
            # Option 1:
            # Generate softmax only for samples recorded during training.
            # =========================================================
            if args.train_paths_file:
                print(
                    f"Generating softmax results from recorded training paths: "
                    f"{args.train_paths_file}"
                )

                # IMPORTANT:
                # Use the same deterministic preprocessing used for evaluation.
                from eyedataset import ImageListDataset
                from pairAttack.generatePair import get_transform
                softmax_transform = get_transform(size=config.get(
                        "model",
                        {},
                    ).get(
                        "input_size",
                ...
                softmax_dataset = ImageListDataset(
                    dataset_root=dataset_path,
                    list_file=args.train_paths_file,
                    transform=softmax_transform,
                )

                softmax_loader = DataLoader(
                    softmax_dataset,
                ...
                print(
                    f"Loaded {len(softmax_dataset)} recorded training samples."
                )
            ...
            # =========================================================
                ...
                print(
                    "No --train_paths_file provided. "
                    "Using the original datatype=0 test_loader."
                )

                if test_loader is None:
                    raise RuntimeError(
                        "test_loader is None; cannot export softmax results."
                    )

                softmax_loader = test_loader

            export_softmax_results(
                model=client.model,
                data_loader=softmax_loader,
                device=device,
                output_path=softmax_output,
                dataset_root=dataset_path,
            )
        ...
        elif args.modedp=="attack":
            attackType = config.get("attackType", "sample")
            if attackType == "hierar_subspace":
                ...
                config = {
                    ...
                    "adv_save_dir": config.get("adv_save_dir", {}),

                    "cache_robust_statistics": True,
                    "log_interval": 50,
                    "save_optimization_process": True,
                    "optimization_process_dir": "/path/to/workspace/program_output/DeepUWF/random_attack_results_robust_subspace_correcttrainset_sample"


                }
                print(f"Running hierarchical subspace attack with config: {config}")
                total_generated, metrics = client.randomAttackRobustSubjectSubspace(config=config)
            elif attackType == "hierar_subspace_pixcons":
                ...
                config = {
                    ...
                    "log_interval": 50,
                    "save_optimization_process": True,
                    "optimization_process_dir": "/path/to/workspace/program_output/DeepUWF/random_attack_results_robust_subspace_correcttrainset_sample_pixcons"


                }
                print(f"Running hierarchical subspace attack with config: {config}")
                total_generated, metrics = client.randomAttackRobustSubjectSubspace_pixcons(config=config)

            else:
                print(f"attackType: {attackType} not implemented")
            print("ddd")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="/path/to/workspace/code/DeepUWF/DP/dp_fl_config_mia.yaml",
    ...
    parser.add_argument(
        ...
        type=str,
        default="softmax",
        help="train, test, attack, or softmax",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Checkpoint path used by test/attack/softmax modes",
    )
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Output JSON path for --modedp softmax. If omitted, "
            "<general.save_dir>/softmax_results.json is used."
        ),
    )
    ...
    parser.add_argument(
        "--train_paths_file",
        type=str,
        default="/path/to/workspace/weights/DeepUWFDP/SHDR_random_attack_results_robust_subspace_moredata/trainset_0.txt",
        help=(
            "TXT file containing relative image paths used during training, "
            "e.g. train_paths/client_0_round_0000.txt"
        ),
    )
