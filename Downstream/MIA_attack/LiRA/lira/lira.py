...
def estimate_loss_distributions(target_data, target_label, shadow_imgs,shadow_labs, num_shadow_models=2, epochs=100, lr=0.01, device='cuda'):
    ...
    # 修复：自动检测数据集的实际类别数（从标签中获取最大值+1）


    # 原始代码：ShadowModel() 使用默认 num_classes=2，stable_phi 使用默认 num_classes=10


    # 修复后：从 shadow_labs 和 target_label 中自动推断实际类别数


    all_labels = torch.cat([shadow_labs, target_label])
    ...
    for omeaga in range(target_data.size(0)):
        ...
        for ind in range(num_shadow_models):
            ...
            # 修复：传递正确的 num_classes 参数


            model_in = ShadowModel(channel=3, num_classes=num_classes).to(device)
            ...
            with torch.no_grad():
                ...
                loss_in = stable_phi(output_in, target_label[omeaga:omeaga+1].to(device), num_classes=num_classes, device=device).cpu().numpy()
            ...
            model_out = ShadowModel(channel=3, num_classes=num_classes).to(device)
            ...
            with torch.no_grad():
                ...
                loss_out = stable_phi(output_out, target_label[omeaga:omeaga+1].to(device), num_classes=num_classes, device=device).cpu().numpy()
...
def membership_inference(model, target_data, target_label, in_mean, in_std, out_mean, out_std, num_classes=10, device='cuda'):
    model.eval()
    with torch.no_grad():
        output = model(target_data.to(device))
        loss = stable_phi(output, target_label.to(device), num_classes=num_classes, device=device).item()
...
def run_over_MIA(model,target_data_col,target_label_col,in_mean_col,in_std_col,out_mean_col,out_std_col,num_classes=10):
    result_col=[]
    for i in range(len(target_data_col)):
        result = membership_inference(model, target_data_col[i:i+1], target_label_col[i:i+1], in_mean_col[i], in_std_col[i], out_mean_col[i], out_std_col[i], num_classes=num_classes)
