"""Dataset loading helpers with fixed sample identifiers."""
...
class BasicDataset(Dataset):
    def __init__(
        ...
        assert split in ['train', 'test', 'val', 'all'], \
            "split must be 'train', 'val', 'test', or 'all'"
        ...
        # 固定随机顺序，确保每次 train/val/test 划分一致
        ...
        logging.info(
            f"Creating {split} dataset with {len(self.ids)} examples "
            f"(total={num_total}, train={len(train_ids)}, "
    ...
    def __getitem__(self, idx):
        ...
        if len(nae_file) != 1:
            raise RuntimeError(
                f"Expected exactly one NAE for name={name}, "
                f"but found {len(nae_file)}: {nae_file}"
            )
        ...
        # torchvision transforms: (height, width)
        target_size = (img.size[1], img.size[0])

        if mask.size != img.size:
            # logging.warning(
            #     f"Mask size mismatch for {name}: "
            #     f"image={img.size}, mask={mask.size}. "
            #     f"Applying Resize and CenterCrop."
            # )
        ...
        if nae.size != img.size:
            # logging.warning(
            #     f"NAE size mismatch for {name}: "
            #     f"image={img.size}, NAE={nae.size}. "
            #     f"Applying Resize and CenterCrop."
            # )

            nae_transform = transforms.Compose([
                transforms.Resize(
                    target_size,
                    interpolation=transforms.InterpolationMode.BICUBIC,
                ),
            ...
            nae = nae_transform(nae)

        if mask.size != img.size:
            raise RuntimeError(
                f"Mask size is still inconsistent after resizing for {name}: "
                f"image={img.size}, mask={mask.size}"
            )

        if nae.size != img.size:
            raise RuntimeError(
                f"NAE size is still inconsistent after resizing for {name}: "
                f"image={img.size}, NAE={nae.size}"
            )
