...
class CW(Attack):
    ...
    def forward(self, images,labels):
        ...
        Overridden.
        """
        _mean_torch = torch.tensor((0.5, 0.5, 0.5)).view(3,1,1).to(self.device)
        _std_torch = torch.tensor((0.5, 0.5, 0.5)).view(3,1,1).to(self.device)
        ...
        #SmooL1 = nn.SmoothL1Loss(reduction='none')
        Flatten = nn.Flatten()
        ...
        for step in range(self.steps):
            ...
            #print("l2 {}".format(L2_loss))
            #current_L1 = SmooL1(Flatten(adv_images),Flatten(images)).sum()
            #adv_images_norm = (adv_images-_mean_torch)/_std_torch
            outputs = self.model(adv_images)
            if self._targeted:
                f_loss = self.f(outputs, target_labels).sum()
                #print("floss {}".format(f_loss))
            else:
                f_loss = self.f(outputs, labels).sum()
            # Update adversarial images
            _, pre = torch.max(outputs.detach(), 1)
            correct = (pre == labels).float()
            #adv_top5,idx = torch.sort(outputs)
            
            #kll = F.kl_div(adv_top5[0][-5:].log(), top5, reduction='sum')
            ...
            if step % (self.steps//10) == 0:
                ...
                prev_cost = cost.item()
        #if self.quan:
            #best_adv_images = torch.round(best_adv_images*255)/255
